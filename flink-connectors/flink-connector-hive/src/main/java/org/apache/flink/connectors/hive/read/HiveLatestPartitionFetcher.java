...
/**
 * Partition fetcher that fetches latest partitions of the given hive table.
 */
public class HiveLatestPartitionFetcher implements PartitionFetcher<HiveTablePartition> {
	...
	public HiveLatestPartitionFetcher(
			DataType[] fieldTypes,
			String[] fieldNames,
			HiveShim shim,
	...
	public List<HiveTablePartition> fetch(Context context) throws Exception {
		...
		if (context.partitionKeys() != null && context.partitionKeys().isEmpty()) {
			...
			allPartValueToTime.sort((o1, o2) -> Long.compare(o2.f1, o1.f1));
			Long latestTime = Long.MIN_VALUE;
			for (Tuple2<List<String>, Long> partValueToTime : allPartValueToTime) {
				if (partValueToTime.f1 >= latestTime) {
				...
				latestTime = Math.max(partValueToTime.f1, latestTime);
			}
		}
