...
public class StreamArrowPythonGroupWindowAggregateFunctionOperator<K, W extends Window>
                ...
                case WINDOW_START:
                    windowProperty.setField(
                            ...
                            TimestampData.fromEpochMillis(
                                    getShiftEpochMills(((TimeWindow) currentWindow).getStart())));
                    break;
                case WINDOW_END:
                    windowProperty.setField(
                            i,
                            TimestampData.fromEpochMillis(
                                    getShiftEpochMills(((TimeWindow) currentWindow).getEnd())));
                    break;
    ...
    private long getShiftEpochMills(long utcTimestampMills) {
        return TimeWindowUtil.toEpochMills(utcTimestampMills, timeZone, false);
    }
    ...
    private class TriggerContext implements Trigger.TriggerContext {
        ...
        public void registerProcessingTimeTimer(long time) {
            internalTimerService.registerProcessingTimeTimer(
                    window, TimeWindowUtil.toEpochMills(time, timeZone, true));
        }
